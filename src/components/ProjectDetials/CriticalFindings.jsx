...
export default function CriticalFindings({ findings = [] }) {
  ...
  return (
    <div className="bg-[#0e1017] border border-rose-900/30 rounded-2xl p-6 space-y-4 font-mono w-full">
      ...
      <div className="space-y-3">
        {findings.map((item, index) => (
          <div key={index} className="bg-rose-950/20 border border-rose-900/40 rounded-xl p-4 space-y-1">
            <h5 className="text-xs font-bold text-rose-400">{item.title}</h5>
