...
export default function ProjectNavbar({ projectName, loading, onSearchChange, searchValue }) {
  ...
  return (
    <header className="w-full bg-[#0a0b0d]/90 backdrop-blur-md border-b border-slate-800/80 px-4 py-1.5 flex items-center justify-between text-slate-300 font-mono text-xs select-none">
      <div className="flex items-center gap-2.5">
        <Link 
          to="/workspaces" 
          className="text-sm font-bold text-slate-100 tracking-tight hover:text-white transition-colors"
        >
          System Atlas
        </Link>
        ...
        <div className="flex items-center gap-1.5 text-[#94a3b8] text-[11px]">
          ...
          <div className="relative flex items-center ml-1">
            ...
            {loading ? (
              <span className="relative text-sm font-extrabold text-slate-500 animate-pulse">
                Loading Project...
              </span>
            ) : (
              <span className="relative text-sm font-extrabold text-white tracking-wide truncate max-w-[280px]">
      ...
      <div className="flex items-center gap-3">
        <div className="relative flex items-center py-5">
          ...
          <input
            ...
            className="w-56 bg-[#12151e] text-slate-200 placeholder-slate-500 text-[11px] pl-7 pr-3 py-1 rounded-full border border-slate-800/90 focus:outline-none focus:border-sky-500/50 transition-colors"
          />
        </div>
