...
export default function TechnologyStack({ techStack = {} }) {
  ...
  return (
    <div className="bg-[#0e1017] border border-slate-800/80 rounded-2xl p-6 space-y-4 w-full">
      ...
      {categories.length === 0 ? (
        <p className="text-xs text-slate-500 font-(family-name:--body-font)">No tech stack data available.</p>
      ) : (
        <div className="space-y-3">
          {categories.map((category) => (
            <div key={category} className="grid grid-cols-4 items-center gap-2 text-xs font-(family-name:--labels)">
              ...
              <div className="col-span-3 flex flex-wrap gap-2">
                {Array.isArray(techStack[category]) ? (
                  techStack[category].map((item, index) => (
                    <span
                      key={index}
                      className="px-2.5 py-1 bg-slate-900 border border-slate-800 rounded-lg text-slate-200 text-xs flex items-center gap-1.5 font-medium"
                    >
                  ...
                  <span className="px-2.5 py-1 bg-slate-900 border border-slate-800 rounded-lg text-slate-200 text-xs font-medium">
