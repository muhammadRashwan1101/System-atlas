...
<div className="mb-4 " >
        <img src={A_logo} alt="logo company" />
       </div>
       <h1 className="my-6 text-4xl font-semibold text-[#D8E2FF]">Visualize Your Architecture</h1>
...
<span className="h-1.5 w-1.5 rounded-full bg-emerald-400 shadow-[0_0_6px_rgba(74,222,128,0.8)]"></span>
    <span className="text-[9px] uppercase tracking-[0.2em] text-gray-400"> {item}</span>
    </div>
))}
 </footer>
 ...
 <div className= " w-full max-w-md flex flex-col items-center gap-4  px-8  ">
                ...
                <form className="flex flex-col gap-7 w-full" onSubmit={handleSubmit(handleLogin)}>
                    <div className="flex flex-col gap-5">
                        <label className="font-(family-name:--labels) text-xs text-[#64748B]" htmlFor="email" >Organization Email</label>
                        <input 
                            className="p-3 bg-[#FFFFFF] rounded-sm focus:outline-none" 
                            id="email" 
                            type="text" 
                        ...
                        />
                        {errors.email && <span className="text-sm text-[#fc3636]">{errors.email.message}</span>}
                    </div>
                    <div className="flex flex-1 flex-col gap-5">
                        <div>
                            <label className="font-(family-name:--labels) text-[#64748B]  text-[11px] " htmlFor="password" >Security Key</label>
                            <Link to="/" className="text-xs text-[#D8E2FFB2] hover:text-(--primary)/80 float-right" >RECOVERY</Link>
                        </div>
                        <input 
                            className="w-full p-3 bg-[#FFFFFF] rounded-sm focus:outline-none"
                            id="password" 
                            type="password" 
  ...
  <label htmlFor="remember" className="text-sm text-[#94A3B8]">
    Trusted device authentication
                    ...
                    <button className="bg-[#ADC6FF] p-5 rounded-md text-[#385283] font-semibold hover:bg-[#ADC6FF]/80 mt-3 flex justify-center gap-3" type="submit">AUTHORIZE ENTRY 
                    <img src={login_icon} alt="" />
                    </button>
                </form>
  <div className="flex flex-col gap-5 my-4 pt-5 justify-center items-center">
    <span className="text-[#64748B] font-light">ENTERPRISE ACCESS RESTRICTED</span>
